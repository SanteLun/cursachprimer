...
ShiftReduceParser::Tokens ShiftReduceParser::getNextToken() {
    ...
    switch (lexType) {
        case 1: return ID;
        case 2: return NUM;
        case 7: // арифметические операторы
...
bool ShiftReduceParser::parse(TLexemList* lexemList, TIdentList* identList, char* errorMessage) {
    ...
    while (success) {
        ...
        if (actionIt == actionTable.end()) {
            ...
            std::stringstream ss;
            ss << "Синтаксическая ошибка в состоянии " << state << " для токена '" << name << "'";
        ...
        switch (action.first) {
            ...
            case REDUCE: {
                ...
                if (stateStack.empty()) {
                    std::ostringstream msg; msg << "Ошибка: пустой стек состояний после свертки";
                ...
                if (gotoIt == gotoTable.end()) {
                    std::stringstream ss;
                    ss << "Ошибка перехода в состоянии " << top;
                    strcpy(errorMessage, ss.str().c_str());
                    return false;
                }
            ...
            case ERROR:
                std::stringstream ss;
                ss << "Неизвестная ошибка в состоянии " << state;
                strcpy(errorMessage, ss.str().c_str());
                return false;
        }
    }
...
bool ShiftReduceParser::parseWithSteps(TLexemList* lexemList, TIdentList* identList, char* errorMessage, std::vector<Step>& steps) {
    ...
    steps.clear();
    // initial record
    steps.push_back(Step(stateStack.top(), "", "инициализация", stackToStr(stateStack)));
    // token mapping
    static const char* tokenNames[] = { "id", "число", "+", "-", "*", "/", "(", ")", "конец" };
    // start parsing
    Tokens token = getNextToken();
    while (true) {
        if (stateStack.empty()) {
            std::ostringstream msg; msg << "Ошибка: пустой стек состояний";
            strcpy(errorMessage, msg.str().c_str());
            steps.push_back(Step(0, "?", "ошибка", "пустой стек"));
            return false;
        }
        int state = stateStack.top();
        ...
        auto actionIt = actionTable.find(std::make_pair(state, token));
        if (actionIt == actionTable.end()) {
            std::ostringstream msg; msg << "Синтаксическая ошибка в состоянии " << state << " для токена '" << tname << "'";
            strcpy(errorMessage, msg.str().c_str());
            steps.push_back(Step(state, tname, "ошибка", stackToStr(stateStack)));
            return false;
        }
        auto action = actionIt->second;
        if (action.first == SHIFT) {
            ...
            {
                std::ostringstream op; op << "сдвиг " << action.second;
            ...
            {
                std::ostringstream op; op << "свертка " << action.second;
                steps.push_back(Step(state, tname, op.str(), stackToStr(stateStack)));
            }
            Rule& rule = rules[action.second - 1];
            for (size_t i = 0; i < rule.rhs.size(); ++i) { 
            ...
            if (stateStack.empty()) {
                std::ostringstream msg; msg << "Ошибка: пустой стек состояний после свертки";
                strcpy(errorMessage, msg.str().c_str());
                steps.push_back(Step(0, tname, "ошибка", "пустой стек"));
                return false;
            }
            int top = stateStack.top();
            auto gtIt = gotoTable.find(std::make_pair(top, rule.lhs));
            if (gtIt == gotoTable.end()) {
                std::ostringstream msg; msg << "Ошибка перехода в состоянии " << top;
                strcpy(errorMessage, msg.str().c_str());
                steps.push_back(Step(top, tname, "ошибка", stackToStr(stateStack)));
                return false;
            }
            stateStack.push(gtIt->second);
            // log goto transition
            {
                std::ostringstream op; op << "переход " << gtIt->second;
                steps.push_back(Step(top, tname, op.str(), stackToStr(stateStack)));
            }
        } else if (action.first == ACCEPT) {
            steps.push_back(Step(state, tname, "принято", stackToStr(stateStack)));
            return true;
        } else { // ERROR action
            std::ostringstream msg; msg << "Ошибка в состоянии " << state;
            strcpy(errorMessage, msg.str().c_str());
            steps.push_back(Step(state, tname, "ошибка", stackToStr(stateStack)));
            return false;
        }
